    ...
    overflow= "hidden" fontWeight={"bold"}   textOverflow= "ellipsis"  fontSize="12px" background="white" width={"70px"} size={2} mt={3} ref={btnRef} onClick={onOpen}>  <u> India </u>
      ...
      >
        ...
        <ModalContent colorScheme='brand'>
          ...
          {data.map((el)=>
          <div >
          <div  style={{display:"flex",fontSize:"10px"}}  key={el.id}> <div  > <img width={"20px"} src={el.flags.png} alt="coutnryFlag" /></div><div style={{marginLeft:"5px"}}><p>{el.name.common}</p></div></div></div>)}
          </ModalBody>
          <ModalFooter>
